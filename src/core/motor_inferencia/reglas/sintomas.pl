% Sintomas

:- dynamic sintoma/1.
sintoma(fiebre).
sintoma(tos).
sintoma(dificultad_respiratoria).
sintoma(dolor_pecho).
sintoma(dolor_espalda).
sintoma(dolor_abdominal).
sintoma(dolor_cabeza).
sintoma(dolor_muscular).
sintoma(dolor_articular).
sintoma(escalofrios).
sintoma(escurrimiento_nasal).
sintoma(mareos).
sintoma(fatiga).
sintoma(vomitos).
sintoma(diarrea).
sintoma(dolor_garganta).
sintoma(inflamacion_amigdalas).
sintoma(dolor_oido).
sintoma(irritacion_oido).
sintoma(disuria).
sintoma(urgencia_miccion).
sintoma(palidez).
sintoma(fotofobia).
sintoma(malestar_general).
sintoma(congestion).
sintoma(sudoracion_nocturna).
sintoma(perdida_peso).
sintoma(erupcion_cutanea).
sintoma(inflamacion_ganglios).
sintoma(fracturas).
sintoma(disminucion_estatura).
sintoma(lagrimeo).
sintoma(secrecion_ocular).
sintoma(picazon_nasal).
sintoma(edemas).
sintoma(cambios_orina).
sintoma(hematuria).
sintoma(conjuntivitis).
sintoma(secrecion).
sintoma(insomnio).
sintoma(cambios_apetito).
sintoma(palpitaciones).
sintoma(tension_muscular).

% Caracteristicas
caracteristica(fiebre, temperatura_alta).
caracteristica(fiebre, sudoracion).
caracteristica(tos, tos_seca).
caracteristica(tos, tos_flema).
caracteristica(dificultad_respiratoria, falta_aire).
caracteristica(dificultad_respiratoria, sibilancias).
caracteristica(dolor_pecho, opresion).
caracteristica(dolor_pecho, punzante).
caracteristica(dolor_espalda, dolor_muscular).
caracteristica(dolor_espalda, dolor_nervioso).
caracteristica(dolor_abdominal, calambres).
caracteristica(dolor_abdominal, dolor_pelvico).
caracteristica(dolor_cabeza, migrana).
caracteristica(dolor_cabeza, tension).
caracteristica(dolor_muscular, debilidad).
caracteristica(dolor_muscular, rigidez).
caracteristica(dolor_articular, inflamacion).
caracteristica(dolor_articular, rigidez).
caracteristica(escalofrios, temblores).
caracteristica(escalofrios, sudoracion).
caracteristica(escurrimiento_nasal, congestion).
caracteristica(escurrimiento_nasal, picazon).
caracteristica(mareos, vertigo).
caracteristica(mareos, desmayo).
caracteristica(fatiga, cansancio).
caracteristica(fatiga, debilidad).
caracteristica(vomitos, nauseas).
caracteristica(vomitos, regurgitacion).
caracteristica(diarrea, heces_liquidas).
caracteristica(diarrea, dolor_abdominal).
caracteristica(diarrea, deshidratacion).
caracteristica(dolor_garganta, irritacion).
caracteristica(dolor_garganta, ardor).
caracteristica(inflamacion_amigdalas, hinchazon).
caracteristica(dolor_oido, pulsatil).
caracteristica(dolor_oido, supuracion).
caracteristica(irritacion_oido, picazon).
caracteristica(disuria, ardor_al_orinar).
caracteristica(urgencia_miccion, necesidad_frecuente).
caracteristica(palidez, disminucion_color_piel).
caracteristica(fotofobia, sensibilidad_luz).
caracteristica(malestar_general, debilidad).
caracteristica(congestion, obstruccion).
caracteristica(sudoracion_nocturna, exceso_sudor).
caracteristica(perdida_peso, reduccion_masa).
caracteristica(erupcion_cutanea, manchas).
caracteristica(inflamacion_ganglios, hinchazon).
caracteristica(fracturas, ruptura_hueso).
caracteristica(disminucion_estatura, acortamiento_vertebral).
caracteristica(lagrimeo, exceso_lagrimas).
caracteristica(secrecion_ocular, pegajosidad).
caracteristica(picazon_nasal, irritacion).
caracteristica(edemas, retencion_liquidos).
caracteristica(cambios_orina, color_alterado).
caracteristica(hematuria, sangre_orina).
caracteristica(dolor_articular, inflamacion_localizada).
caracteristica(secrecion, flujo_excesivo).
caracteristica(insomnio, dificultad_dormir).
caracteristica(cambios_apetito, aumento_o_disminucion).
caracteristica(palpitaciones, latidos_acelerados).
caracteristica(tension_muscular, contraccion_muscular).